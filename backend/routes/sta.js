...
router.get('/', protect, async (req, res) => {
  try {
    ...
    if (search) {
      query.$or = [
        { taName: { $regex: search, $options: 'i' } },
        { cityMun: { $regex: search, $options: 'i' } },
        { typeCode: { $regex: search, $options: 'i' } }
      ];
    }
    const total = await STA.countDocuments(query);
    const records = await STA.find(query)
...
router.get('/:id', protect, async (req, res) => {
  try {
    const record = await STA.findById(req.params.id).populate('submittedBy', 'name email');
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const record = await STA.create(data);
...
router.post('/bulk', protect, async (req, res) => {
  try {
    ...
    const created = await STA.insertMany(records);
...
router.put('/:id', protect, async (req, res) => {
  try {
    const record = await STA.findById(req.params.id);
    if (!record) return res.status(404).json({ success: false, message: 'Record not found' });
    if (req.user.role !== 'admin' && record.submittedBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: 'Not authorized' });
    }
    const updated = await STA.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    await STA.findByIdAndDelete(req.params.id);
