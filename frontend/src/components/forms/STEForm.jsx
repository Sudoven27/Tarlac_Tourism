...
const STE_TYPES = ['Travel and Tours', 'Restaurant', 'Transportation', 'Souvenir Shop', 'Spa and Wellness', 'Entertainment', 'Others'];
const MONTHS = ['January','February','March','April','May','June','July','August','September','October','November','December'];
const CLASSIFICATIONS = ['Small', 'Medium', 'Large', 'Micro', 'Travel Organizer', 'Fine Dining', 'Casual Dining', 'Fast Food', 'Carinderia', 'Others'];

const INITIAL = {
  nameOfEnterprise: '', attractionCode: '', type: '', seatsUnit: '',
  totalEmployees: '', femaleEmployees: '', maleEmployees: '',
  reportYear: new Date().getFullYear(), reportMonth: new Date().getMonth() + 1,
  region: 'Region III', provHuc: 'Tarlac', cityMun: '',
  classification: '', contactNumber: '', email: '', address: '', notes: '',
  images: []
};

export default function STEForm({ initial, onSubmit, loading }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="space-y-6">
      ...
      <div>
        <p className="section-label">Tourism Enterprise Info (Form STE1)</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="form-label">Name of Enterprise <span className="text-red-500">*</span></label>
            <input className="form-input" value={form.nameOfEnterprise}
              onChange={e => set('nameOfEnterprise', e.target.value)}
              placeholder="Complete official name" required />
          </div>
          <div>
            ...
            <input className="form-input" value={form.attractionCode}
              ...
              placeholder="e.g. TE-2026-001" maxLength={30} />
          </div>
          <div>
            ...
            <select className="form-select" value={form.type} onChange={e => set('type', e.target.value)} required>
              ...
              {STE_TYPES.map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="form-label">Seats / Units</label>
            <input className="form-input" type="number" min="0" value={form.seatsUnit}
              onChange={e => set('seatsUnit', e.target.value)} placeholder="0" />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input className="form-input" type="number" min="0" value={form.totalEmployees}
              onChange={e => set('totalEmployees', e.target.value)} placeholder="0" />
          </div>
          <div>
          ...
          <div>
            ...
            <select className="form-select" value={form.classification} onChange={e => set('classification', e.target.value)}>
      ...
      <button type="submit" className="btn-primary w-full justify-center py-3" disabled={loading}>
        {loading ? <><div className="spinner w-4 h-4" /> Saving...</> : 'Save STE Record'}
      </button>
    </form>
  );
}
